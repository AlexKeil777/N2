...
**Styles** (`assets/css/styles.css`): Single file using CSS custom properties in `:root` for all theme colors and spacing. "Lab Datasheet" theme — light bone-paper background (`#f2ede1`) with a faint graph-paper grid, near-black ink text (`#1b1a16`), burnt-amber primary accent (`#c1531f`), deep ink-teal secondary accent (`#2c6055`), hairline rules, boxy low-radius geometry, and IBM Plex Mono for labels/kickers/data. Flat and printed — no glass/blur or glowing gradients. Responsive breakpoints at 640px (mobile) and 1100px (tablet). NOTE: the self-contained pages that don't load `styles.css` (`v4_continuous_vs_batch_animation.html`) and the case-study pages (`Tiramisu_Calculator_v1_3.html`, `validation_plan.html`, `validation plan da gpt.html`, `user_guide.html`) still carry the older dark theme in their own inline CSS.
